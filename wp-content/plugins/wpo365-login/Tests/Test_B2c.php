    ...
    class Test_B2c
    {
        ...
        public function test_decode_id_token()
        {
            ...
            if (empty($this->id_token)) {
                $error_message = isset($_POST['error_description']) ? \sanitize_text_field($_POST['error_description']) : 'Could not process the ID token. Please check the <a href="#debug">debug log</a> for errors.';

                if (false !== WordPress_Helpers::stripos($error_message, 'AADB2C90057')) {
                    $application_id = Options_Service::get_aad_option('application_id');
        ...
        public function test_source_for_extra_fields()
        {
            ...
            $test_result = new Test_Result('Use ID token as source for custom user fields (instead of Microsoft Graph)', Test_Result::CAPABILITY_PROFILE_PLUS, Test_Result::SEVERITY_LOW);
            $test_result->passed = true;

            $extra_user_fields_source = Options_Service::get_global_string_var('extra_user_fields_source');

            if (0 !== strcasecmp($extra_user_fields_source, 'idToken')) {
                $test_result->passed = false;
                $test_result->message = 'Starting with v18.0 it is recommended - when using Azure AD B2C - to select the ID token (and its custom claims) as source for extra user fields. This will prevent the plugin from trying to retrieve a user-resource from Microsoft Graph.';
                $test_result->more_info = 'https://docs.wpo365.com/article/130-azure-ad-b2c-based-single-sign-on-for-wordpress';
                $test_result->fix = array(
                    array(
                        'op' => 'replace',
                        'value' => array(
                            'extraUserFieldsSource' => 'idToken',
                        ),
                    ),
                );
            }
