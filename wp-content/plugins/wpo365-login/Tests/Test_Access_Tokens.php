...
if (!class_exists('\Wpo\Tests\Test_Access_Tokens')) {
    ...
    class Test_Access_Tokens
    {
        ...
        public function test_roles_access_delegated()
        {
            ...
            $suitable_extensions = \array_flip(array('wpo365-login-premium/wpo365-login.php', 'wpo365-login-intranet/wpo365-login.php', 'wpo365-roles-access/wpo365-roles-access.php'));
        ...
        public function test_avatar_delegated()
        {
            ...
            $suitable_extensions = \array_flip(array('wpo365-login-premium/wpo365-login.php', 'wpo365-login-intranet/wpo365-login.php', 'wpo365-avatar/wpo365-avatar.php'));
        ...
        public function test_scim_application()
        {
            ...
            $suitable_extensions = \array_flip(array('wpo365-login-intranet/wpo365-login.php', 'wpo365-scim/wpo365-scim.php'));
            ...
            if (!defined('WPO_SCIM_TOKEN') || empty(constant('WPO_SCIM_TOKEN'))) {
                $test_result->passed = false;
        ...
        public function test_sharepoint_search_delegated()
        {
            ...
            if (!$sharepoint_endpoint_ok) {
                ...
                $test_result_sites_read_all = $this->check_static_permission(
                    ...
                    'https://docs.wpo365.com/article/126-sharepoint-library-for-wordpress'
                );
            ...
            if (!$sites_endpoint_ok) {
                ...
                $test_result->fix = array(
                    array(
                        ...
                        'value' => array(
                            'graphAllowedEndpoints' => array(
                                'key' => 'https://graph.microsoft.com/_/sites',
                                'boolVal' => false,
                            ),
                        ),
                    ),
            ...
            if (!$drives_endpoint_ok) {
                ...
                $test_result->fix = array(
                    array(
                        ...
                        'value' => array(
                            'graphAllowedEndpoints' => array(
                                'key' => 'https://graph.microsoft.com/_/drives',
                                'boolVal' => false,
                            ),
                        ),
                    ),
            ...
            if (is_wp_error($this->delegated_access_token) || !property_exists($this->delegated_access_token, 'access_token')) {
                ...
                $this->delegated_static_permissions = array_map(
                    function ($item) {
                        ...
                        return str_replace('https://graph.microsoft.com/', '', $lowered);
                    },
                    $this->delegated_static_permissions
                );
            }
         ...
         */
        private function get_application_access_token($scope = 'https://graph.microsoft.com/.default', $role = null)
        {
        ...
        {
            $permission = str_replace('https://graph.microsoft.com/', '', $permission);
